// This program demonstrates friends of a class with a static class member variable.
#include <iostream> 
#include <iomanip>
#include "budget3.h" 
using namespace std;
...
int main()
{
   ...
   for (int count = 0; count < N_DIVISIONS; count++)
   {
    ...
    // Print total Division Requests
    cout << "\tTotal Division Requests: $ ";
    double totalDivBudget = 0;
    for (int count = 0; count < N_DIVISIONS; count++) {
        totalDivBudget += divisions[count].getDivBudget();
    }
    cout << totalDivBudget << endl;

    // Print total Auxillary Requests
    cout << "\tTotal Auxillary Requests: $ ";
     double totalAuxBudget = 0;
    for (int count = 0; count < N_DIVISIONS; count++) {
        totalAuxBudget += auxOffices[count].getDivBudget();
    }
    cout << totalAuxBudget << endl;
